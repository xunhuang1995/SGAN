...
parser.add_argument('--out_dir', type=str, default='logs/gan1')
...
train_batch_disc = th.function(inputs=[x, y, y_1hot, lr],
                               outputs=[loss_disc1_class, loss_disc1_adv, gen_fc3, real_fc3],
                               updates=disc1_param_updates+disc1_bn_updates)
train_batch_gen = th.function(inputs=[y_1hot, lr],
                              outputs=[loss_gen1_adv, loss_gen1_cond, loss_gen1_ent],
                              updates=gen1_param_updates+gen1_bn_updates)
...
for epoch in range(args.num_epoch):
    ...
    for t in range(nr_batches_train):
        ...
        l_disc1_class, l_disc1_adv, g1, r1 = \
                train_batch_disc(batchx, batchy, batchy_1hot, args.disc_lr)

        ''' train generators '''
        # l_disc_adv = min(l_disc0_adv, l_disc1_adv)
        if l_disc1_adv > 0.65: # discriminator is random guessing
            gen0_iter = 1
        elif l_disc1_adv > 0.5: # discriminator works reasonably well (60%)
            gen0_iter = 3
        elif l_disc1_adv > 0.3: # discriminator very strong (74%)
            gen0_iter = 5
        else:
            gen0_iter = 7
        for i in range(gen0_iter): # train geneartor for three times
            l_gen1_adv, l_gen1_cond, l_gen1_ent = \
                    train_batch_gen(batchy_1hot, args.gen_lr)
        ...
        print("Epoch %d, time = %ds, var gen fc3 = %.4f, var real fc3 = %.4f" %
             (epoch, time.time()-begin, np.var(np.array(g1)), np.var(np.array(r1))))
    ...
    if epoch%args.save_interval==0:
        np.savez(args.out_dir + "/disc1_params_epoch{}.npz".format(epoch), *LL.get_all_param_values(disc1_layers))
        np.savez(args.out_dir + '/gen1_params_epoch{}.npz'.format(epoch), *LL.get_all_param_values(gen1_layers))
